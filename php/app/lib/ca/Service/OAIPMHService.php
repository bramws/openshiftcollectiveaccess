...
require_once(__CA_LIB_DIR__."/ca/ImportExport/OAIPMH/Provider/OaiIdentifier.php");
...
class OAIPMHService extends BaseService {
	...
	private function getRecord($oaiData) {
		...
		$t_item = $o_dm->getInstanceByTableName($vs_table, true);
		...
		$va_items = $o_exporter->export($this->getMappingCode(), $t_item, null, array('returnDom' => false));
		...
		$o_doc_src = DomDocument::loadXML(array_pop($va_items));
    ...
    # -------------------------------------------------------
    /**
     * Returns an array of "DataMover" metadata format processing classes
     *
     * @return array Array indexed on classname (which is the same as the format's short name), with format class instances as values.
     */
    private function getMetadataFormats() {
        $o_dir = new DirectoryIterator(__CA_LIB_DIR__.'/ca/ImportExport/Formats');
        $va_metadata_formats = array();
        $va_configured_formats = is_array($this->opa_provider_info['formats']) ? array_keys($this->opa_provider_info['formats']) : array();
        foreach($va_configured_formats as $vn_i => $vs_format) {
        	$va_configured_formats[$vn_i] = strtolower($va_configured_formats[$vn_i]);
        }
        foreach ($o_dir as $o_entry) {
    		$vs_filename = $o_entry->getFilename();
    		$vs_pathname = $o_entry->getPathname();
        	if ($o_entry->isDir() && file_exists($vs_pathname.'/'.$vs_filename.'.php')) {
        		require_once(__CA_LIB_DIR__.'/ca/ImportExport/Formats/'.$vs_filename.'/'.$vs_filename.'.php');
        		$vs_classname = 'DataMover'.$vs_filename;
        		if ($o_format = new $vs_classname) {
        			if (method_exists($o_format, "getMetadataPrefix") && in_array(strtolower($o_format->getMetadataPrefix()), $va_configured_formats)) {
        				$va_metadata_formats[$vs_filename] = $o_format;
        			}
        		}
        	}
        }
	 ...
	 *
	 * @return bool Alwaysreturns true
	 */
	...
	private function checkParameters($pa_required_parameters, $pa_optional_parameters) {
        ...
        if ($vs_metadata_prefix) {
       		foreach($this->getMetadataFormats() as $vs_name => $o_format) {
       			if ($vs_metadata_prefix == $o_format->getMetadataPrefix()) {
	...
	public function getMappingCode() {
		...
		if ($ps_metadata_prefix && isset($this->opa_provider_info['formats'][$ps_metadata_prefix]) && $this->opa_provider_info['formats'][$ps_metadata_prefix]) {
			return $this->opa_provider_info['formats'][$ps_metadata_prefix];
		}
		return $this->opa_provider_info['formats'][$this->opa_provider_info['default_format']];
	}	
	# -------------------------------------------------------	
}
?>